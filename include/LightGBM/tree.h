...
public:
  ...
  * \param zero_bin bin value for value==0
  * \param default_bin bin value of default value
  * \param default_value Split gain
...
inline int Tree::GetLeaf(const double* feature_values) const {
  ...
  if (has_categorical_) {
    while (node >= 0) {
      double fval = DefaultValueForZero<double>(feature_values[split_feature_[node]], 0, default_value_[node]);
